...
class ViewController: UIViewController, UIPickerViewDelegate, UIPickerViewDataSource, UITextFieldDelegate, UITableViewDataSource, UITableViewDelegate  {
    ...
    @IBAction func addNameButton(sender: UIButton) {
        ...
        playerList.append(Player(name: nameToAdd!))
        nameTable.reloadData()
    }
    
    //Generate Teams button
    @IBAction func generateTeams(sender: AnyObject) {
        teamController = TeamController(players: playerList, teamSize: selectedTeamSize)
    }
    ...
    func tableView(tableView: UITableView, cellForRowAtIndexPath indexPath: NSIndexPath) -> UITableViewCell {
        ...
        currentCell.textLabel?.text = playerList[indexPath.row].Name
        return currentCell
    }
}
